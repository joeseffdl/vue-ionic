<template>
  <ion-modal ref="modal" trigger="open-modal" :can-dismiss="canDismiss">
    <div class="grid justify-items-center p-6">
      <ion-icon
        src="src/assets/svg/confirmation.svg"
        class="size-[71px] mb-[14px]"
        aria-hidden="true"
      ></ion-icon>
      ...
      <ion-button :router-link="`${routes.CHECKOUT}`" @click="dismiss()"
        >Proceed to Checkout</ion-button
      >
      <ion-text
        class="h-[19px] mt-2 text-[--ion-color-primary] font-bold text-sm leading-[19.1px] font-[Nunito]"
        :router-link="routes.PRODUCT"
        @click="dismiss()"
        >Add More</ion-text
      >
    </div>
...
const routes = reactive({
  PRODUCT: "/products",
  CHECKOUT: `/products/${slug}/checkout`,
});
